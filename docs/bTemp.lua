...
local bc=ac:gsub(".lua","")bb[bc]=_b(bc)end end;return bb end
...
cb[ad]={}end;table.insert(cb[ad],bd)end end end end end
if(fs.exists(ac))then
for cc,dc in pairs(fs.list(ac))do table.insert(db,dc)
local _d=_b(dc:gsub(".lua",""))
if(type(_d)=="table")then for ad,bd in pairs(_d)do
if(type(ad)=="string")then
if(cb[ad]==nil)then cb[ad]={}end;table.insert(cb[ad],bd)end end end end end;local function bc(cc)return cb[cc]end
return
...
a_a.__index=a_a;cd=setmetatable(a_a,cd)end;return cd end end end;return cc end}end;ba["plugins"]={}
ba["plugins"]["dynamicValues"]=function(...)
local bb=_b("utils")local cb=bb.tableCount;local db=bb.xmlValue
return
...
acb:animateOffset(d_c,_ac,aac,bac,cac)end;return acb end}return _cb end}end
...
local bd={}bd.___value=nil;bd.___name=ad;bd.___children={}bd.___props={}
bd.___reactiveProps={}function bd:value()return self.___value end
function bd:setValue(cd)self.___value=cd end;function bd:name()return self.___name end
function bd:setName(cd)self.___name=cd end;function bd:children()return self.___children end;function bd:numChildren()return
#self.___children end
function bd:addChild(cd)
if self[cd:name()]~=nil then
if
type(self[cd:name()].name)=="function"then local dd={}
table.insert(dd,self[cd:name()])self[cd:name()]=dd end;table.insert(self[cd:name()],cd)else
self[cd:name()]=cd end;table.insert(self.___children,cd)end;function bd:properties()return self.___props end;function bd:numProperties()
return#self.___props end
function bd:addProperty(cd,dd)local __a="@"..cd
if self[__a]~=nil then if
type(self[__a])=="string"then local a_a={}table.insert(a_a,self[__a])
self[__a]=a_a end
table.insert(self[__a],dd)else self[__a]=dd end
table.insert(self.___props,{name=cd,value=self[__a]})end;function bd:reactiveProperties()return self.___reactiveProps end;function bd:addReactiveProperty(cd,dd)
self.___reactiveProps[cd]=dd end;return bd end;local ac={}
function ac:ToXmlString(ad)ad=string.gsub(ad,"&","&amp;")
ad=string.gsub(ad,"<","&lt;")ad=string.gsub(ad,">","&gt;")
ad=string.gsub(ad,"\"","&quot;")
ad=string.gsub(ad,"([^%w%&%;%p%\t% ])",function(bd)
return string.format("&#x%X;",string.byte(bd))end)return ad end
function ac:FromXmlString(ad)
ad=string.gsub(ad,"&#x([%x]+)%;",function(bd)
return string.char(tonumber(bd,16))end)
ad=string.gsub(ad,"&#([0-9]+)%;",function(bd)return string.char(tonumber(bd,10))end)ad=string.gsub(ad,"&quot;","\"")
ad=string.gsub(ad,"&apos;","'")ad=string.gsub(ad,"&gt;",">")
ad=string.gsub(ad,"&lt;","<")ad=string.gsub(ad,"&amp;","&")return ad end;function ac:ParseProps(ad,bd)
string.gsub(bd,"(%w+)=([\"'])(.-)%2",function(cd,dd,__a)
ad:addProperty(cd,self:FromXmlString(__a))end)end;function ac:ParseReactiveProps(ad,bd)
string.gsub(bd,"(%w+)={(.-)}",function(cd,dd)
ad:addReactiveProperty(cd,dd)end)end
function ac:ParseXmlText(ad)local bd={}
local cd=_c()table.insert(bd,cd)local dd,__a,a_a,b_a,c_a;local d_a,_aa=1,1
while true do
dd,_aa,__a,a_a,b_a,c_a=string.find(ad,"<(%/?)([%w_:]+)(.-)(%/?)>",d_a)if not dd then break end;local baa=string.sub(ad,d_a,dd-1)
if not
string.find(baa,"^%s*$")then
local caa=(cd:value()or"")..self:FromXmlString(baa)bd[#bd]:setValue(caa)end
if c_a=="/"then local caa=_c(a_a)self:ParseProps(caa,b_a)
self:ParseReactiveProps(caa,b_a)cd:addChild(caa)elseif __a==""then local caa=_c(a_a)
self:ParseProps(caa,b_a)self:ParseReactiveProps(caa,b_a)
table.insert(bd,caa)cd=caa else local caa=table.remove(bd)cd=bd[#bd]
if#bd<1 then error("XmlParser: nothing to close with "..
a_a)end;if caa:name()~=a_a then
error("XmlParser: trying to close "..caa.name.." with "..a_a)end;cd:addChild(caa)end;d_a=_aa+1 end;local aaa=string.sub(ad,d_a)if#bd>1 then
error("XmlParser: unclosed "..bd[#bd]:name())end;return cd end;local function bc(ad,bd)local cd=db('script',ad)if(cd~=nil)then
load(cd,nil,"t",bd.env)()end end
local function cc(ad,bd,cd,dd)
local __a=dd.env
if(bd:sub(1,1)=="$")then local a_a=bd:sub(2)
cd(ad,ad:getBasalt().getVariable(a_a))else
cd(ad,function(...)__a.event={...}local a_a,b_a=pcall(load(bd,nil,"t",__a))if
not a_a then error("XML Error: "..b_a)end end)end end;local dc=nil
local _d=function(ad)
for bd,cd in ipairs(ad.dependencies)do for dd,__a in ipairs(cd)do if(__a==ad)then
table.remove(cd,dd)end end end;ad.dependencies={}end
return
{basalt=function(ad)
local bd={reactive=function(cd)local dd=cd;local __a={}
local a_a=function()if(dc~=nil)then table.insert(__a,dc)
table.insert(dc.dependencies,__a)end;return dd end
local b_a=function(c_a)dd=c_a;local d_a={}for _aa,aaa in ipairs(__a)do d_a[_aa]=aaa end;for _aa,aaa in ipairs(d_a)do
aaa.execute()end end;return a_a,b_a end,untracked=function(cd)
local dd=dc;dc=nil;local __a=cd()dc=dd;return __a end,effect=function(cd)
local dd={dependencies={}}
local __a=function()_d(dd)local a_a=dc;dc=dd;cd()dc=a_a end;dd.execute=__a;dd.execute()end,derived=function(cd)
local dd,__a=ad.reactive()ad.effect(function()__a(cd())end)return dd end}return bd end,VisualObject=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;local b_a,c_a=dd:getPosition()
local d_a,_aa=dd:getSize()
if(__a=="x")then dd:setPosition(a_a,c_a)elseif(__a=="y")then
dd:setPosition(b_a,a_a)elseif(__a=="width")then dd:setSize(a_a,_aa)elseif(__a=="height")then
dd:setSize(d_a,a_a)elseif(__a=="background")then dd:setBackground(colors[a_a])elseif
(__a=="foreground")then dd:setForeground(colors[a_a])end end,updateSpecifiedValuesByXMLData=function(dd,__a,a_a)for b_a,c_a in
ipairs(a_a)do local d_a=db(c_a,__a)
if(d_a~=nil)then dd:updateValue(c_a,d_a)end end end,setValuesByXMLData=function(dd,__a,a_a)
a_a.env[dd:getName()]=dd
for c_a,d_a in pairs(__a:reactiveProperties())do
local _aa=function()
local aaa=load("return "..d_a,nil,"t",a_a.env)()dd:updateValue(c_a,aaa)end;bd.effect(_aa)end
dd:updateSpecifiedValuesByXMLData(__a,{"x","y","width","height","background","foreground"})
local b_a={"onClick","onClickUp","onHover","onScroll","onDrag","onKey","onKeyUp","onRelease","onChar","onGetFocus","onLoseFocus","onResize","onReposition","onEvent","onLeave"}for c_a,d_a in pairs(b_a)do if(db(d_a,__a)~=nil)then
cc(dd,db(d_a,__a),dd[d_a],a_a)end end
return dd end}return cd end,ChangeableObject=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)if(__a=="value")then
dd:setValue(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)dd:updateSpecifiedValuesByXMLData(__a,{"value"})if(
db("onChange",__a)~=nil)then
cc(dd,db("onChange",__a),dd.onChange,a_a)end;return dd end}return cd end,Container=function(ad,bd)
local cd={}local function dd(b_a,c_a,d_a)
if(c_a~=nil)then c_a:setValuesByXMLData(b_a,d_a)end end
local function __a(b_a,c_a,d_a,_aa)
if(b_a~=nil)then if(b_a.properties~=nil)then
b_a={b_a}end
for aaa,baa in pairs(b_a)do
local caa=c_a(d_a,baa["@id"]or cb())cd[caa:getName()]=caa;dd(baa,caa,_aa)end end end
local a_a={setValuesByXMLData=function(b_a,c_a,d_a)cd={}ad.setValuesByXMLData(b_a,c_a,d_a)
local _aa=c_a:children()local aaa=bd.getObjects()
for baa,caa in pairs(_aa)do local daa=caa.___name;if(daa~="animation")then
local _ba=daa:gsub("^%l",string.upper)
if(aaa[_ba]~=nil)then local aba=b_a["add".._ba]__a(caa,aba,b_a,d_a)end end end;__a(c_a["animation"],b_a.addAnimation,b_a,d_a)return
b_a end,loadLayout=function(b_a,c_a,d_a)
if
(fs.exists(c_a))then local _aa={}_aa.env=_ENV;_aa.env.props=d_a;local aaa=fs.open(c_a,"r")
local baa=ac:ParseXmlText(aaa.readAll())aaa.close()cd={}bc(baa,_aa)
b_a:setValuesByXMLData(baa,_aa)end;return b_a end,getXMLElements=function(b_a)return
cd end}return a_a end,BaseFrame=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getOffset()if(__a=="layout")then dd:setLayout(a_a)elseif(__a=="xOffset")then
dd:setOffset(a_a,c_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"layout","xOffset"})return dd end}return cd end,Frame=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getOffset()
if(__a=="layout")then dd:setLayout(a_a)elseif(__a=="xOffset")then
dd:setOffset(a_a,c_a)elseif(__a=="yOffset")then dd:setOffset(b_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"layout","xOffset","yOffset"})return dd end}return cd end,Flexbox=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if
(__a=="flexDirection")then dd:setFlexDirection(a_a)elseif(__a=="justifyContent")then
dd:setJustifyContent(a_a)elseif(__a=="alignItems")then dd:setAlignItems(a_a)elseif(__a=="spacing")then
dd:setSpacing(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"flexDirection","justifyContent","alignItems","spacing"})return dd end}return cd end,Button=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if(__a=="text")then
dd:setText(a_a)elseif(__a=="horizontalAlign")then dd:setHorizontalAlign(a_a)elseif
(__a=="verticalAlign")then dd:setVerticalAlign(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"text","horizontalAlign","verticalAlign"})return dd end}return cd end,Label=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if(__a=="text")then
dd:setText(a_a)elseif(__a=="align")then dd:setAlign(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"text","align"})return dd end}return cd end,Input=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a,d_a=dd:getDefaultText()
if(__a=="defaultText")then dd:setDefaultText(a_a,c_a,d_a)elseif
(__a=="defaultFG")then dd:setDefaultText(b_a,a_a,d_a)elseif(__a=="defaultBG")then
dd:setDefaultText(b_a,c_a,a_a)elseif(__a=="offset")then dd:setOffset(a_a)elseif(__a=="textOffset")then
dd:setTextOffset(a_a)elseif(__a=="text")then dd:setText(a_a)elseif(__a=="inputLimit")then
dd:setInputLimit(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"defaultText","defaultFG","defaultBG","offset","textOffset","text","inputLimit"})return dd end}return cd end,Image=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getOffset()
if(__a=="xOffset")then dd:setOffset(a_a,c_a)elseif(__a=="yOffset")then
dd:setOffset(b_a,a_a)elseif(__a=="path")then dd:loadImage(a_a)elseif(__a=="usePalette")then
dd:usePalette(a_a)elseif(__a=="play")then dd:play(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"xOffset","yOffset","path","usePalette","play"})return dd end}return cd end,Checkbox=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getSymbol()
if(__a=="text")then dd:setText(a_a)elseif(__a=="checked")then dd:setChecked(a_a)elseif(
__a=="textPosition")then dd:setTextPosition(a_a)elseif(__a=="activeSymbol")then
dd:setSymbol(a_a,c_a)elseif(__a=="inactiveSymbol")then dd:setSymbol(b_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"text","checked","textPosition","activeSymbol","inactiveSymbol"})return dd end}return cd end,Program=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)if
(__a=="execute")then dd:execute(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"execute"})return dd end}return cd end,Progressbar=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a,d_a=dd:getProgressBar()
if(__a=="direction")then dd:setDirection(a_a)elseif(__a=="activeBarColor")then
dd:setProgressBar(a_a,c_a,d_a)elseif(__a=="activeBarSymbol")then dd:setProgressBar(b_a,a_a,d_a)elseif(__a==
"activeBarSymbolColor")then dd:setProgressBar(b_a,c_a,a_a)elseif
(__a=="backgroundSymbol")then dd:setBackgroundSymbol(a_a)elseif(__a=="progress")then
dd:setProgress(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"direction","activeBarColor","activeBarSymbol","activeBarSymbolColor","backgroundSymbol","progress"})return dd end}return cd end,Slider=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if(__a=="symbol")then
dd:setSymbol(a_a)elseif(__a=="symbolColor")then dd:setSymbolColor(a_a)elseif(__a=="index")then
dd:setIndex(a_a)elseif(__a=="maxValue")then dd:setMaxValue(a_a)elseif(__a=="barType")then
dd:setBarType(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"symbol","symbolColor","index","maxValue","barType"})return dd end}return cd end,Scrollbar=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if(__a=="symbol")then
dd:setSymbol(a_a)elseif(__a=="symbolColor")then dd:setSymbolColor(a_a)elseif(__a=="symbolSize")then
dd:setSymbolSize(a_a)elseif(__a=="scrollAmount")then dd:setScrollAmount(a_a)elseif(__a=="index")then
dd:setIndex(a_a)elseif(__a=="maxValue")then dd:setMaxValue(a_a)elseif(__a=="barType")then
dd:setBarType(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"symbol","symbolColor","symbolSize","scrollAmount","index","maxValue","barType"})return dd end}return cd end,MonitorFrame=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)if
(__a=="monitor")then dd:setMonitor(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"monitor"})return dd end}return cd end,Switch=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if(__a=="symbol")then
dd:setSymbol(a_a)elseif(__a=="activeBackground")then dd:setActiveBackground(a_a)elseif
(__a=="inactiveBackground")then dd:setInactiveBackground(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"symbol","activeBackground","inactiveBackground"})return dd end}return cd end,Textfield=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getSelection()local d_a,_aa=dd:getOffset()
if(__a=="bgSelection")then
dd:setSelection(b_a,a_a)elseif(__a=="fgSelection")then dd:setSelection(a_a,c_a)elseif(__a=="xOffset")then
dd:setOffset(a_a,_aa)elseif(__a=="yOffset")then dd:setOffset(d_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"bgSelection","fgSelection","xOffset","yOffset"})
if(__a["lines"]~=nil)then local b_a=__a["lines"]["line"]if
(b_a.properties~=nil)then b_a={b_a}end;for c_a,d_a in pairs(b_a)do
dd:addLine(d_a:value())end end
if(__a["keywords"]~=nil)then
for b_a,c_a in pairs(__a["keywords"])do
if(colors[b_a]~=nil)then
local d_a=c_a;if(d_a.properties~=nil)then d_a={d_a}end;local _aa={}
for aaa,baa in pairs(d_a)do
local caa=baa["keyword"]if(baa["keyword"].properties~=nil)then
caa={baa["keyword"]}end;for daa,_ba in pairs(caa)do
table.insert(_aa,_ba:value())end end;dd:addKeywords(colors[b_a],_aa)end end end
if(__a["rules"]~=nil)then
if(__a["rules"]["rule"]~=nil)then
local b_a=__a["rules"]["rule"]if(__a["rules"]["rule"].properties~=nil)then
b_a={__a["rules"]["rule"]}end
for c_a,d_a in pairs(b_a)do if(db("pattern",d_a)~=nil)then
dd:addRule(db("pattern",d_a),colors[db("fg",d_a)],colors[db("bg",d_a)])end end end end;return dd end}return cd end,Thread=function(ad,bd)
local cd={setValuesByXMLData=function(dd,__a,a_a)local b_a=
db("start",__a)~=nil;if(b_a~=nil)then
local c_a=load(b_a,nil,"t",a_a.env)dd:start(c_a)end;return dd end}return cd end,Timer=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)if(__a=="start")then
dd:start(a_a)elseif(__a=="time")then dd:setTime(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"start","time"})if(db("onCall",__a)~=nil)then
cc(dd,db("onCall",__a),dd.onCall,a_a)end;return dd end}return cd end,List=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getSelectionColor()
if(__a=="align")then dd:setTextAlign(a_a)elseif(__a=="offset")then
dd:setOffset(a_a)elseif(__a=="selectionBg")then dd:setSelectionColor(a_a,c_a)elseif
(__a=="selectionFg")then dd:setSelectionColor(b_a,a_a)elseif(__a=="scrollable")then
dd:setScrollable(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"align","offset","selectionBg","selectionFg","scrollable"})
if(__a["item"]~=nil)then local b_a=__a["item"]
if(b_a.properties~=nil)then b_a={b_a}end
for c_a,d_a in pairs(b_a)do if(dd:getType()~="Radio")then
dd:addItem(db("text",d_a),colors[db("bg",d_a)],colors[db("fg",d_a)])end end end;return dd end}return cd end,Dropdown=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getDropdownSize()
if(__a=="dropdownWidth")then dd:setDropdownSize(a_a,c_a)elseif
(__a=="dropdownHeight")then dd:setDropdownSize(b_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"dropdownWidth","dropdownHeight"})return dd end}return cd end,Radio=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getBoxSelectionColor()local d_a,_aa=dd:setBoxDefaultColor()
if(__a=="selectionBg")then
dd:setBoxSelectionColor(a_a,c_a)elseif(__a=="selectionFg")then dd:setBoxSelectionColor(b_a,a_a)elseif
(__a=="defaultBg")then dd:setBoxDefaultColor(a_a,_aa)elseif(__a=="defaultFg")then
dd:setBoxDefaultColor(d_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"selectionBg","selectionFg","defaultBg","defaultFg"})
if(__a["item"]~=nil)then local b_a=__a["item"]
if(b_a.properties~=nil)then b_a={b_a}end;for c_a,d_a in pairs(b_a)do
dd:addItem(db("text",d_a),db("x",d_a),db("y",d_a),colors[db("bg",d_a)],colors[db("fg",d_a)])end end;return dd end}return cd end,Menubar=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
if(__a=="space")then
dd:setSpace(a_a)elseif(__a=="scrollable")then dd:setScrollable(a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"space","scrollable"})return dd end}return cd end,Graph=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getGraphSymbol()
if(__a=="maxEntries")then dd:setMaxEntries(a_a)elseif(__a=="type")then
dd:setType(a_a)elseif(__a=="minValue")then dd:setMinValue(a_a)elseif(__a=="maxValue")then
dd:setMaxValue(a_a)elseif(__a=="symbol")then dd:setGraphSymbol(a_a,c_a)elseif(__a=="symbolColor")then
dd:setGraphSymbol(b_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"maxEntries","type","minValue","maxValue","symbol","symbolColor"})
if(__a["item"]~=nil)then local b_a=__a["item"]
if(b_a.properties~=nil)then b_a={b_a}end
for c_a,d_a in pairs(b_a)do dd:addDataPoint(db("value"))end end;return dd end}return cd end,Treeview=function(ad,bd)
local cd={updateValue=function(dd,__a,a_a)if(
a_a==nil)then return end;ad.updateValue(dd,__a,a_a)
local b_a,c_a=dd:getSelectionColor()local d_a,_aa=dd:getOffset()
if(__a=="space")then dd:setSpace(a_a)elseif
(__a=="scrollable")then dd:setScrollable(a_a)elseif(__a=="selectionBg")then
dd:setSelectionColor(a_a,c_a)elseif(__a=="selectionFg")then dd:setSelectionColor(b_a,a_a)elseif(__a=="xOffset")then
dd:setOffset(a_a,_aa)elseif(__a=="yOffset")then dd:setOffset(d_a,a_a)end end,setValuesByXMLData=function(dd,__a,a_a)
ad.setValuesByXMLData(dd,__a,a_a)
dd:updateSpecifiedValuesByXMLData(__a,{"space","scrollable","selectionBg","selectionFg","xOffset","yOffset"})
local function b_a(c_a,d_a)
if(d_a["node"]~=nil)then local _aa=d_a["node"]
if(_aa.properties~=nil)then _aa={_aa}end
for aaa,baa in pairs(_aa)do
local caa=c_a:addNode(db("text",baa),colors[db("bg",baa)],colors[db("fg",baa)])b_a(caa,baa)end end end
if(__a["node"]~=nil)then local c_a=__a["node"]
if(c_a.properties~=nil)then c_a={c_a}end
for d_a,_aa in pairs(c_a)do
local aaa=dd:addNode(db("text",_aa),colors[db("bg",_aa)],colors[db("fg",_aa)])b_a(aaa,_aa)end end;return dd end}return cd end}end;ba["libraries"]={}
ba["libraries"]["basaltDraw"]=function(...)
local bb=_b("tHex")local cb=_b("utils")local db=cb.splitString;local _c,ac=string.sub,string.rep
return
function(bc)local cc=bc or
term.current()local dc;local _d,ad=cc.getSize()local bd={}local cd={}local dd={}
local __a;local a_a={}local function b_a()__a=ac(" ",_d)
for n=0,15 do local daa=2 ^n;local _ba=bb[daa]a_a[daa]=ac(_ba,_d)end end;b_a()
local function c_a()b_a()local daa=__a
local _ba=a_a[colors.white]local aba=a_a[colors.black]
for currentY=1,ad do
bd[currentY]=_c(
bd[currentY]==nil and daa or
bd[currentY]..daa:sub(1,_d-bd[currentY]:len()),1,_d)
...
return d_a(_ca,dba(aca,_c))end end;_d.__index=_d;return setmetatable(_d,ac)end end;return ba["main"]()