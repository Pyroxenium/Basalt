...
bd.parent:addEvent("other_event",bd)bd.parent:addEvent("mouse_drag",bd)end end}return setmetatable(ad,_b)end
end; 
...
project['default']['Frame'] = function(...)local db=require("Object")local _c=require("loadObjects")
local ac=require("basaltDraw")local bc=require("utils")local cc=require("layout")
local dc=require("basaltMon")local _d=bc.uuid;local ad=bc.rpairs;local bd=bc.getValueFromXML;local cd=bc.tableCount
local dd,__a,a_a=string.sub,math.min,math.max
return
function(b_a,c_a,d_a,_aa)local aaa=db(b_a)local baa="Frame"local caa={}local daa={}local _ba={}local aba={}local bba={}local cba={}
local dba={}local _ca={}local aca=0;local bca=d_a or term.current()local cca=""local dca=false
local _da=false;local ada=false;local bda=0;local cda=0;local dda=false;local __b=0;local a_b=false;local b_b=false;local c_b=""local d_b=false
local _ab=false;local aab;local bab;local cab=true;local dab=true;local _bb=false;local abb={}aaa:setZIndex(10)
local bbb=ac(bca)local cbb=false;local dbb=1;local _cb=1;local acb=colors.white;local bcb,ccb=0,0;local dcb={}local function _db(cbc,dbc)if(dbc~=nil)then
dbc:setValuesByXMLData(cbc)end end
local function adb(cbc,dbc,_cc)
if(cbc~=nil)then if(
cbc.properties~=nil)then cbc={cbc}end
for acc,bcc in pairs(cbc)do
local ccc=dbc(_cc,bcc["@id"]or _d())table.insert(dcb,ccc)_db(bcc,ccc)end end end
local function bdb(cbc)for dbc,_cc in pairs(caa)do
for acc,bcc in pairs(_cc)do if(bcc:getName()==cbc)then return bcc end end end end
local function cdb(cbc)local dbc=bdb(cbc)if(dbc~=nil)then return dbc end
for _cc,acc in pairs(caa)do
for bcc,ccc in pairs(acc)do if(
ccc:getType()=="Frame")then local dcc=ccc:getDeepObject(cbc)
if(dcc~=nil)then return dcc end end end end end
local function ddb(cbc)local dbc=cbc:getZIndex()
if(bdb(cbc.name)~=nil)then return nil end
if(caa[dbc]==nil)then
for x=1,#daa+1 do if(daa[x]~=nil)then if(dbc==daa[x])then break end;if
(dbc>daa[x])then table.insert(daa,x,dbc)break end else
table.insert(daa,dbc)end end;if(#daa<=0)then table.insert(daa,dbc)end;caa[dbc]={}end;cbc.parent=_ba;if(cbc.init~=nil)then cbc:init()end
table.insert(caa[dbc],cbc)return cbc end
local function __c(cbc,dbc)
for _cc,acc in pairs(aba)do
for bcc,ccc in pairs(acc)do
for dcc,_dc in pairs(ccc)do
if(_dc==dbc)then
table.remove(aba[_cc][bcc],dcc)if(cbc.parent~=nil)then if(cd(aba[_cc])<=0)then
cbc.parent:removeEvent(_cc,cbc)end end end end end end end
local function a_c(cbc,dbc)
for _cc,acc in pairs(caa)do
for bcc,ccc in pairs(acc)do
if(type(dbc)=="string")then
if(ccc:getName()==dbc)then
table.remove(caa[_cc],bcc)__c(_ba,ccc)cbc:updateDraw()return true end else if(ccc==dbc)then table.remove(caa[_cc],bcc)__c(_ba,ccc)
cbc:updateDraw()return true end end end end;return false end;local function b_c(cbc,dbc,_cc)
for acc,bcc in pairs(aba[dbc])do for ccc,dcc in pairs(bcc)do
if(dcc:getName()==_cc)then return dcc end end end end
local function c_c(cbc,dbc,_cc)
local acc=_cc:getZIndex()if(aba[dbc]==nil)then aba[dbc]={}end;if(bba[dbc]==nil)then
bba[dbc]={}end
if(b_c(cbc,dbc,_cc.name)~=nil)then return nil end
if(cbc.parent~=nil)then cbc.parent:addEvent(dbc,cbc)end;abb[dbc]=true
if(aba[dbc][acc]==nil)then
for x=1,#bba[dbc]+1 do
if
(bba[dbc][x]~=nil)then if(acc==bba[dbc][x])then break end;if(acc>bba[dbc][x])then
table.insert(bba[dbc],x,acc)break end else
table.insert(bba[dbc],acc)end end
if(#bba[dbc]<=0)then table.insert(bba[dbc],acc)end;aba[dbc][acc]={}end;table.insert(aba[dbc][acc],_cc)return _cc end
local function d_c(cbc,dbc,_cc)
if(aba[dbc]~=nil)then
for acc,bcc in pairs(aba[dbc])do
for ccc,dcc in pairs(bcc)do
if(dcc==_cc)then
table.remove(aba[dbc][acc],ccc)if(#aba[dbc][acc]<=0)then aba[dbc][acc]=nil
if(cbc.parent~=nil)then if(
cd(aba[dbc])<=0)then abb[dbc]=false
cbc.parent:removeEvent(dbc,cbc)end end end;return
true end end end end;return false end
local function _ac(cbc)local dbc,_cc=pcall(load("return "..cbc))
if not(dbc)then error(cbc..
" is not a valid dynamic code")end;return load("return "..cbc)()end
local function aac(cbc,dbc,_cc)for acc,bcc in pairs(_ca)do
if(bcc[2]==_cc)and(bcc[4]==dbc)then return bcc end end;aca=aca+1
_ca[aca]={0,_cc,{},dbc,aca}return _ca[aca]end
local function bac(cbc,dbc)local _cc={}local acc={}for bcc in dbc:gmatch("%a+%.x")do local ccc=bcc:gsub("%.x","")
if
(ccc~="self")and(ccc~="parent")then table.insert(_cc,ccc)end end
for bcc in
dbc:gmatch("%w+%.y")do local ccc=bcc:gsub("%.y","")if(ccc~="self")and(ccc~="parent")then
table.insert(_cc,ccc)end end;for bcc in dbc:gmatch("%a+%.w")do local ccc=bcc:gsub("%.w","")
if(ccc~="self")and
(ccc~="parent")then table.insert(_cc,ccc)end end
for bcc in
dbc:gmatch("%a+%.h")do local ccc=bcc:gsub("%.h","")if(ccc~="self")and(ccc~="parent")then
table.insert(_cc,ccc)end end
for bcc,ccc in pairs(_cc)do acc[ccc]=bdb(ccc)if(acc[ccc]==nil)then
error("Dynamic Values - unable to find object "..ccc)end end;acc["self"]=cbc;acc["parent"]=cbc:getParent()return acc end
local function cac(cbc,dbc)local _cc=cbc;for acc in cbc:gmatch("%w+%.x")do
_cc=_cc:gsub(acc,dbc[acc:gsub("%.x","")]:getX())end;for acc in cbc:gmatch("%w+%.y")do
_cc=_cc:gsub(acc,dbc[acc:gsub("%.y","")]:getY())end;for acc in cbc:gmatch("%w+%.w")do
_cc=_cc:gsub(acc,dbc[acc:gsub("%.w","")]:getWidth())end;for acc in cbc:gmatch("%w+%.h")do
_cc=_cc:gsub(acc,dbc[acc:gsub("%.h","")]:getHeight())end;return _cc end
local function dac(cbc)
if(#_ca>0)then
for n=1,aca do
if(_ca[n]~=nil)then local dbc;if(#_ca[n][3]<=0)then
_ca[n][3]=bac(_ca[n][4],_ca[n][2])end
dbc=cac(_ca[n][2],_ca[n][3])_ca[n][1]=_ac(dbc)if(_ca[n][4]:getType()=="Frame")then
_ca[n][4]:recalculateDynamicValues()end end end
for dbc,_cc in pairs(daa)do if(caa[_cc]~=nil)then
for acc,bcc in pairs(caa[_cc])do if(bcc.eventHandler~=nil)then
bcc:eventHandler("dynamicValueEvent",cbc)end end end end end end;local function _bc(cbc)return _ca[cbc][1]end
local function abc(cbc)
for dbc,_cc in pairs(caa)do
for acc,bcc in pairs(_cc)do
if
(bcc.getHeight~=nil)and(bcc.getY~=nil)then
local ccc,dcc=bcc:getHeight(),bcc:getY()if(ccc+dcc-cbc:getHeight()>__b)then
__b=a_a(ccc+dcc-cbc:getHeight(),0)end end end end end;local function bbc(cbc)
if(bab~=aab)then if(bab~=nil)then bab:loseFocusHandler()end;if
(aab~=nil)then aab:getFocusHandler()end;bab=aab end end
_ba={barActive=false,barBackground=colors.gray,barTextcolor=colors.black,barText="New Frame",barTextAlign="left",addEvent=c_c,removeEvent=d_c,removeEvents=__c,getEvent=b_c,newDynamicValue=aac,recalculateDynamicValues=dac,getDynamicValue=_bc,getType=function(cbc)return
baa end,setFocusedObject=function(cbc,dbc)aab=dbc;bbc(cbc)return cbc end,getVariable=function(cbc,dbc)return
_aa.getVariable(dbc)end,setSize=function(cbc,dbc,_cc,acc)
aaa.setSize(cbc,dbc,_cc,acc)if(cbc.parent==nil)then bbb=ac(bca)end
for bcc,ccc in pairs(daa)do if(caa[ccc]~=nil)then
for dcc,_dc in
pairs(caa[ccc])do if(_dc.eventHandler~=nil)then
_dc:eventHandler("basalt_resize",_dc,cbc)end end end end;cbc:recalculateDynamicValues()cab=false;return cbc end,setTheme=function(cbc,dbc,_cc)
if(
type(dbc)=="table")then dba=dbc elseif(type(dbc)=="string")then dba[dbc]=_cc end;cbc:updateDraw()return cbc end,getTheme=function(cbc,dbc)
return
dba[dbc]or(cbc.parent~=nil and cbc.parent:getTheme(dbc)or
_aa.getTheme(dbc))end,setPosition=function(cbc,dbc,_cc,acc)
aaa.setPosition(cbc,dbc,_cc,acc)
for bcc,ccc in pairs(daa)do if(caa[ccc]~=nil)then
for dcc,_dc in pairs(caa[ccc])do if(_dc.eventHandler~=nil)then
_dc:eventHandler("basalt_reposition",_dc,cbc)end end end end;cbc:recalculateDynamicValues()return cbc end,getBasaltInstance=function(cbc)return
_aa end,setOffset=function(cbc,dbc,_cc)
bcb=dbc~=nil and
math.floor(dbc<0 and math.abs(dbc)or-dbc)or bcb
ccb=_cc~=nil and
math.floor(_cc<0 and math.abs(_cc)or-_cc)or ccb;cbc:updateDraw()return cbc end,getOffsetInternal=function(cbc)return
bcb,ccb end,getOffset=function(cbc)
return bcb<0 and math.abs(bcb)or-bcb,
ccb<0 and math.abs(ccb)or-ccb end,removeFocusedObject=function(cbc)aab=nil;bbc(cbc)return cbc end,getFocusedObject=function(cbc)return
bab end,setCursor=function(cbc,dbc,_cc,acc,bcc)
if(cbc.parent~=nil)then
local ccc,dcc=cbc:getAnchorPosition()
cbc.parent:setCursor(dbc or false,(_cc or 0)+ccc-1,(acc or 0)+dcc-1,
bcc or acb)else
local ccc,dcc=cbc:getAbsolutePosition(cbc:getAnchorPosition(cbc:getX(),cbc:getY(),true))cbb=dbc or false;if(_cc~=nil)then dbb=ccc+_cc-1 end;if(acc~=nil)then _cb=dcc+
acc-1 end;acb=bcc or acb;if(cbb)then
bca.setTextColor(acb)bca.setCursorPos(dbb,_cb)bca.setCursorBlink(cbb)else
bca.setCursorBlink(false)end end;return cbc end,setMovable=function(cbc,dbc)
if(
cbc.parent~=nil)then d_b=dbc or not d_b
cbc.parent:addEvent("mouse_click",cbc)abb["mouse_click"]=true
cbc.parent:addEvent("mouse_up",cbc)abb["mouse_up"]=true
cbc.parent:addEvent("mouse_drag",cbc)abb["mouse_drag"]=true end;return cbc end,setScrollable=function(cbc,dbc)dda=(
dbc or dbc==nil)and true or false
if(
cbc.parent~=nil)then cbc.parent:addEvent("mouse_scroll",cbc)end;abb["mouse_scroll"]=true;return cbc end,setScrollAmount=function(cbc,dbc)__b=
dbc or __b;dab=false;return cbc end,getScrollAmount=function(cbc)return
dab and __b or abc(cbc)end,show=function(cbc)aaa.show(cbc)
if(cbc.parent==nil)then
_aa.setActiveFrame(cbc)
if(dca)and not(_da)then _aa.setMonitorFrame(cca,cbc)elseif(_da)then
_aa.setMonitorFrame(cbc:getName(),cbc,cca)else _aa.setMainFrame(cbc)end end;return cbc end,hide=function(cbc)
aaa.hide(cbc)
if(cbc.parent==nil)then if(activeFrame==cbc)then activeFrame=nil end
if(dca)and
not(_da)then if(_aa.getMonitorFrame(cca)==cbc)then
_aa.setActiveFrame(nil)end elseif(_da)then
if(
_aa.getMonitorFrame(cbc:getName())==cbc)then _aa.setActiveFrame(nil)end else
if(_aa.getMainFrame()==cbc)then _aa.setMainFrame(nil)end end end;return cbc end,addLayout=function(cbc,dbc)
if(
dbc~=nil)then
if(fs.exists(dbc))then local _cc=fs.open(dbc,"r")
local acc=cc:ParseXmlText(_cc.readAll())_cc.close()dcb={}cbc:setValuesByXMLData(acc)end end;return cbc end,getLastLayout=function(cbc)return
dcb end,addLayoutFromString=function(cbc,dbc)if(dbc~=nil)then local _cc=cc:ParseXmlText(dbc)
cbc:setValuesByXMLData(_cc)end;return cbc end,setValuesByXMLData=function(cbc,dbc)
aaa.setValuesByXMLData(cbc,dbc)if(bd("movable",dbc)~=nil)then if(bd("movable",dbc))then
cbc:setMovable(true)end end;if(
bd("scrollable",dbc)~=nil)then
if(bd("scrollable",dbc))then cbc:setScrollable(true)end end;if
(bd("monitor",dbc)~=nil)then
cbc:setMonitor(bd("monitor",dbc)):show()end;if(bd("mirror",dbc)~=nil)then
cbc:setMirror(bd("mirror",dbc))end
if(bd("bar",dbc)~=nil)then if(bd("bar",dbc))then
cbc:showBar(true)else cbc:showBar(false)end end
if(bd("barText",dbc)~=nil)then cbc.barText=bd("barText",dbc)end;if(bd("barBG",dbc)~=nil)then
cbc.barBackground=colors[bd("barBG",dbc)]end;if(bd("barFG",dbc)~=nil)then
cbc.barTextcolor=colors[bd("barFG",dbc)]end;if(bd("barAlign",dbc)~=nil)then
cbc.barTextAlign=bd("barAlign",dbc)end;if(bd("layout",dbc)~=nil)then
cbc:addLayout(bd("layout",dbc))end;if(bd("xOffset",dbc)~=nil)then
cbc:setOffset(bd("xOffset",dbc),ccb)end;if(bd("yOffset",dbc)~=nil)then
cbc:setOffset(ccb,bd("yOffset",dbc))end;if(bd("scrollAmount",dbc)~=nil)then
cbc:setScrollAmount(bd("scrollAmount",dbc))end;local _cc=dbc:children()
for acc,bcc in
pairs(_cc)do if(bcc.___name~="animation")then
local ccc=bcc.___name:gsub("^%l",string.upper)
if(_c[ccc]~=nil)then adb(bcc,cbc["add"..ccc],cbc)end end end;adb(dbc["frame"],cbc.addFrame,cbc)
adb(dbc["animation"],cbc.addAnimation,cbc)return cbc end,showBar=function(cbc,dbc)cbc.barActive=
dbc or not cbc.barActive;cbc:updateDraw()
return cbc end,setBar=function(cbc,dbc,_cc,acc)cbc.barText=dbc or""cbc.barBackground=
_cc or cbc.barBackground
cbc.barTextcolor=acc or cbc.barTextcolor;cbc:updateDraw()return cbc end,setBarTextAlign=function(cbc,dbc)cbc.barTextAlign=
dbc or"left"cbc:updateDraw()return cbc end,setMirror=function(cbc,dbc)if(
cbc.parent~=nil)then
error("Frame has to be a base frame in order to attach a mirror.")end;c_b=dbc;if(mirror~=nil)then
bbb.setMirror(mirror)end;a_b=true;return cbc end,removeMirror=function(cbc)mirror=
nil;a_b=false;bbb.setMirror(nil)return cbc end,setMonitorScale=function(cbc,dbc)if
(dca)then bca.setTextScale(dbc)end;return cbc end,setMonitor=function(cbc,dbc,_cc)
if(
dbc~=nil)and(dbc~=false)then
if(type(dbc)=="string")then
if(
peripheral.getType(dbc)=="monitor")then bca=peripheral.wrap(dbc)ada=true end
if(cbc.parent~=nil)then cbc.parent:removeObject(cbc)end;dca=true;_aa.setMonitorFrame(dbc,cbc)elseif(type(dbc)=="table")then
bca=dc(dbc)ada=true;dca=true;_da=true
_aa.setMonitorFrame(cbc:getName(),cbc,true)end else bca=parentTerminal;dca=false;_da=false
if(type(cca)=="string")then
if(
_aa.getMonitorFrame(cca)==cbc)then _aa.setMonitorFrame(cca,nil)end else
if(_aa.getMonitorFrame(cbc:getName())==cbc)then _aa.setMonitorFrame(cbc:getName(),
nil)end end end;if(_cc~=nil)then bca.setTextScale(_cc)end;bbb=ac(bca)
cbc:setSize(bca.getSize())cab=true;cca=dbc or nil;cbc:updateDraw()return cbc end,loseFocusHandler=function(cbc)
aaa.loseFocusHandler(cbc)if(bab~=nil)then bab:loseFocusHandler()bab=nil end end,getFocusHandler=function(cbc)
aaa.getFocusHandler(cbc)
if(cbc.parent~=nil)then
if(d_b)then cbc.parent:removeEvents(cbc)
cbc.parent:removeObject(cbc)cbc.parent:addObject(cbc)for dbc,_cc in pairs(abb)do if(_cc)then
cbc.parent:addEvent(dbc,cbc)end end
cbc:updateDraw()end end;if(bab~=nil)then bab:getFocusHandler()end end,eventHandler=function(cbc,dbc,_cc,acc,bcc,ccc)
aaa.eventHandler(cbc,dbc,_cc,acc,bcc,ccc)
if(aba["other_event"]~=nil)then
for dcc,_dc in ipairs(bba["other_event"])do
if(
aba["other_event"][_dc]~=nil)then for adc,bdc in ad(aba["other_event"][_dc])do
if
(bdc.eventHandler~=nil)then if(bdc:eventHandler(dbc,_cc,acc,bcc,ccc))then return true end end end end end end;if(cab)and not(dca)then
if(cbc.parent==nil)then if(dbc=="term_resize")then
cbc:setSize(bca.getSize())cab=true end end end
if(dca)then
if(cab)then
if
(dbc=="monitor_resize")then
if(type(cca)=="string")then cbc:setSize(bca.getSize())elseif(
type(cca)=="table")then
for dcc,_dc in pairs(cca)do for adc,bdc in pairs(_dc)do if(_cc==bdc)then
cbc:setSize(bca.getSize())end end end end;cab=true;cbc:updateDraw()end end
if(dbc=="peripheral")and(_cc==cca)then if
(peripheral.getType(cca)=="monitor")then ada=true;bca=peripheral.wrap(cca)bbb=ac(bca)
cbc:updateDraw()end end
if(dbc=="peripheral_detach")and(_cc==cca)then ada=false end end
if(a_b)then if(peripheral.getType(c_b)=="monitor")then b_b=true
bbb.setMirror(peripheral.wrap(c_b))end;if(dbc=="peripheral_detach")and
(_cc==c_b)then ada=false end
if
(dbc=="monitor_touch")and(c_b==_cc)then cbc:mouseHandler(1,acc,bcc,true)end end
if(dbc=="terminate")and(cbc.parent==nil)then _aa.stop()end end,mouseHandler=function(cbc,dbc,_cc,acc,bcc,ccc)
if
(_da)then if(bca.calculateClick~=nil)then
_cc,acc=bca.calculateClick(ccc,_cc,acc)end end
if(aaa.mouseHandler(cbc,dbc,_cc,acc))then
if(aba["mouse_click"]~=nil)then
cbc:setCursor(false)
for dcc,_dc in ipairs(bba["mouse_click"])do
if
(aba["mouse_click"][_dc]~=nil)then for adc,bdc in ad(aba["mouse_click"][_dc])do
if(bdc.mouseHandler~=nil)then if
(bdc:mouseHandler(dbc,_cc,acc))then bbc(cbc)return true end end end end end end
if(d_b)then
local dcc,_dc=cbc:getAbsolutePosition(cbc:getAnchorPosition())if
(_cc>=dcc)and(_cc<=dcc+cbc:getWidth()-1)and(acc==_dc)then _ab=true;bda=dcc-_cc
cda=yOff and 1 or 0 end end;cbc:removeFocusedObject()return true end;return false end,mouseUpHandler=function(cbc,dbc,_cc,acc)if
(_ab)then _ab=false end
if(aaa.mouseUpHandler(cbc,dbc,_cc,acc))then
if
(aba["mouse_up"]~=nil)then
for bcc,ccc in ipairs(bba["mouse_up"])do
if(aba["mouse_up"][ccc]~=nil)then
for dcc,_dc in
ad(aba["mouse_up"][ccc])do if(_dc.mouseUpHandler~=nil)then
if(_dc:mouseUpHandler(dbc,_cc,acc))then bbc(cbc)return true end end end end end end;bbc(cbc)return true end;return false end,scrollHandler=function(cbc,dbc,_cc,acc)
if
(aaa.scrollHandler(cbc,dbc,_cc,acc))then
if(aba["mouse_scroll"]~=nil)then
for ccc,dcc in pairs(bba["mouse_scroll"])do
if(
aba["mouse_scroll"][dcc]~=nil)then
for _dc,adc in ad(aba["mouse_scroll"][dcc])do if(adc.scrollHandler~=
nil)then
if(adc:scrollHandler(dbc,_cc,acc))then bbc(cbc)return true end end end end end end;local bcc=ccb
if(dda)then abc(cbc)if(dbc>0)or(dbc<0)then
ccb=a_a(__a(ccb-dbc,0),-__b)cbc:updateDraw()end end;cbc:removeFocusedObject()if(ccb==bcc)then return false end
return true end;return false end,dragHandler=function(cbc,dbc,_cc,acc)
if
(_ab)then local bcc,ccc=cbc.parent:getOffsetInternal()bcc=bcc<0 and
math.abs(bcc)or-bcc;ccc=
ccc<0 and math.abs(ccc)or-ccc;local dcc=1;local _dc=1;if(cbc.parent~=nil)then
dcc,_dc=cbc.parent:getAbsolutePosition(cbc.parent:getAnchorPosition())end
cbc:setPosition(
_cc+bda- (dcc-1)+bcc,acc+cda- (_dc-1)+ccc)cbc:updateDraw()return true end
if(cbc:isVisible())and(cbc:isEnabled())then
if
(aba["mouse_drag"]~=nil)then
for bcc,ccc in ipairs(bba["mouse_drag"])do
if
(aba["mouse_drag"][ccc]~=nil)then for dcc,_dc in ad(aba["mouse_drag"][ccc])do
if(_dc.dragHandler~=nil)then if
(_dc:dragHandler(dbc,_cc,acc))then bbc(cbc)return true end end end end end end end;bbc(cbc)aaa.dragHandler(cbc,dbc,_cc,acc)return false end,keyHandler=function(cbc,dbc,_cc)
if
(cbc:isFocused())or(cbc.parent==nil)then
local acc=cbc:getEventSystem():sendEvent("key",cbc,"key",dbc)if(acc==false)then return false end
if(aba["key"]~=nil)then
for bcc,ccc in pairs(bba["key"])do
if(
aba["key"][ccc]~=nil)then
for dcc,_dc in ad(aba["key"][ccc])do if(_dc.keyHandler~=nil)then if
(_dc:keyHandler(dbc,_cc))then return true end end end end end end end;return false end,keyUpHandler=function(cbc,dbc)
if
(cbc:isFocused())or(cbc.parent==nil)then
local _cc=cbc:getEventSystem():sendEvent("key_up",cbc,"key_up",dbc)if(_cc==false)then return false end
if(aba["key_up"]~=nil)then
for acc,bcc in
pairs(bba["key_up"])do
if(aba["key_up"][bcc]~=nil)then for ccc,dcc in ad(aba["key_up"][bcc])do
if(
dcc.keyUpHandler~=nil)then if(dcc:keyUpHandler(dbc))then return true end end end end end end end;return false end,charHandler=function(cbc,dbc)
if
(cbc:isFocused())or(cbc.parent==nil)then
local _cc=cbc:getEventSystem():sendEvent("char",cbc,"char",dbc)if(_cc==false)then return false end
if(aba["char"]~=nil)then
for acc,bcc in
pairs(bba["char"])do
if(aba["char"][bcc]~=nil)then for ccc,dcc in ad(aba["char"][bcc])do
if
(dcc.charHandler~=nil)then if(dcc:charHandler(dbc))then return true end end end end end end end;return false end,setText=function(cbc,dbc,_cc,acc)
local bcc,ccc=cbc:getAnchorPosition()
if(_cc>=1)and(_cc<=cbc:getHeight())then
if(cbc.parent~=nil)then
cbc.parent:setText(a_a(
dbc+ (bcc-1),bcc),ccc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)))else
bbb.setText(a_a(dbc+ (bcc-1),bcc),ccc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)))end end end,setBG=function(cbc,dbc,_cc,acc)
local bcc,ccc=cbc:getAnchorPosition()
if(_cc>=1)and(_cc<=cbc:getHeight())then
if(cbc.parent~=nil)then
cbc.parent:setBG(a_a(
dbc+ (bcc-1),bcc),ccc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)))else
bbb.setBG(a_a(dbc+ (bcc-1),bcc),ccc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)))end end end,setFG=function(cbc,dbc,_cc,acc)
local bcc,ccc=cbc:getAnchorPosition()
if(_cc>=1)and(_cc<=cbc:getHeight())then
if(cbc.parent~=nil)then
cbc.parent:setFG(a_a(
dbc+ (bcc-1),bcc),ccc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)))else
bbb.setFG(a_a(dbc+ (bcc-1),bcc),ccc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)))end end end,writeText=function(cbc,dbc,_cc,acc,bcc,ccc)
local dcc,_dc=cbc:getAnchorPosition()
if(_cc>=1)and(_cc<=cbc:getHeight())then
if(cbc.parent~=nil)then
cbc.parent:writeText(a_a(
dbc+ (dcc-1),dcc),_dc+_cc-1,dd(acc,a_a(1 -dbc+1,1),
cbc:getWidth()-dbc+1),bcc,ccc)else
bbb.writeText(a_a(dbc+ (dcc-1),dcc),_dc+_cc-1,dd(acc,a_a(1 -dbc+1,1),a_a(
cbc:getWidth()-dbc+1,1)),bcc,ccc)end end end,drawBackgroundBox=function(cbc,dbc,_cc,acc,bcc,ccc)
local dcc,_dc=cbc:getAnchorPosition()
bcc=(_cc<1 and(
bcc+_cc>cbc:getHeight()and cbc:getHeight()or bcc+_cc-1)or(
bcc+
_cc>cbc:getHeight()and cbc:getHeight()-_cc+1 or bcc))
acc=(dbc<1 and(acc+dbc>cbc:getWidth()and cbc:getWidth()or acc+
dbc-1)or(

acc+dbc>cbc:getWidth()and cbc:getWidth()-dbc+1 or acc))
if(cbc.parent~=nil)then
cbc.parent:drawBackgroundBox(a_a(dbc+ (dcc-1),dcc),a_a(_cc+ (_dc-1),_dc),acc,bcc,ccc)else
bbb.drawBackgroundBox(a_a(dbc+ (dcc-1),dcc),a_a(_cc+ (_dc-1),_dc),acc,bcc,ccc)end end,drawTextBox=function(cbc,dbc,_cc,acc,bcc,ccc)
local dcc,_dc=cbc:getAnchorPosition()
bcc=(_cc<1 and(
bcc+_cc>cbc:getHeight()and cbc:getHeight()or bcc+_cc-1)or(
bcc+
...
acc+dbc>cbc:getWidth()and cbc:getWidth()-dbc+1 or acc))
if(cbc.parent~=nil)then
cbc.parent:drawTextBox(a_a(dbc+ (dcc-1),dcc),a_a(_cc+ (_dc-1),_dc),acc,bcc,dd(ccc,1,1))else
bbb.drawTextBox(a_a(dbc+ (dcc-1),dcc),a_a(_cc+ (_dc-1),_dc),acc,bcc,dd(ccc,1,1))end end,drawForegroundBox=function(cbc,dbc,_cc,acc,bcc,ccc)
local dcc,_dc=cbc:getAnchorPosition()
bcc=(_cc<1 and(
bcc+_cc>cbc:getHeight()and cbc:getHeight()or bcc+_cc-1)or(
bcc+
...
acc+dbc>cbc:getWidth()and cbc:getWidth()-dbc+1 or acc))
if(cbc.parent~=nil)then
cbc.parent:drawForegroundBox(a_a(dbc+ (dcc-1),dcc),a_a(_cc+ (_dc-1),_dc),acc,bcc,ccc)else
bbb.drawForegroundBox(a_a(dbc+ (dcc-1),dcc),a_a(_cc+ (_dc-1),_dc),acc,bcc,ccc)end end,draw=function(cbc,dbc)if
(dca)and not(ada)then return false end
if(cbc.parent==nil)then if(cbc:getDraw()==
false)then return false end end
if(aaa.draw(cbc))then
local _cc,acc=cbc:getAbsolutePosition(cbc:getAnchorPosition())local bcc,ccc=cbc:getAnchorPosition()local dcc,_dc=cbc:getSize()
if(
cbc.parent==nil)then
if(cbc.bgColor~=false)then
bbb.drawBackgroundBox(bcc,ccc,dcc,_dc,cbc.bgColor)bbb.drawTextBox(bcc,ccc,dcc,_dc," ")end;if(cbc.fgColor~=false)then
bbb.drawForegroundBox(bcc,ccc,dcc,_dc,cbc.fgColor)end end
if(cbc.barActive)then
if(cbc.parent~=nil)then
cbc.parent:writeText(bcc,ccc,bc.getTextHorizontalAlign(cbc.barText,dcc,cbc.barTextAlign),cbc.barBackground,cbc.barTextcolor)else
bbb.writeText(bcc,ccc,bc.getTextHorizontalAlign(cbc.barText,dcc,cbc.barTextAlign),cbc.barBackground,cbc.barTextcolor)end
if(cbc:getBorder("left"))then
if(cbc.parent~=nil)then
cbc.parent:drawBackgroundBox(bcc-1,ccc,1,1,cbc.barBackground)if(cbc.bgColor~=false)then
cbc.parent:drawBackgroundBox(bcc-1,ccc+1,1,_dc-1,cbc.bgColor)end end end
if(cbc:getBorder("top"))then if(cbc.parent~=nil)then
cbc.parent:drawBackgroundBox(bcc-1,ccc-1,dcc+1,1,cbc.barBackground)end end end;for adc,bdc in ad(daa)do
if(caa[bdc]~=nil)then for cdc,ddc in pairs(caa[bdc])do
if(ddc.draw~=nil)then ddc:draw()end end end end end end,updateTerm=function(cbc)if
(dca)and not(ada)then return false end;bbb.update()end,addObject=function(cbc,dbc)return
ddb(dbc)end,removeObject=a_c,getObject=function(cbc,dbc)return bdb(dbc)end,getDeepObject=function(cbc,dbc)return cdb(dbc)end,addFrame=function(cbc,dbc)local _cc=_aa.newFrame(
dbc or _d(),cbc,nil,_aa)return ddb(_cc)end,init=function(cbc)
if
not(_bb)then
if(c_a~=nil)then aaa.width,aaa.height=c_a:getSize()
cbc:setBackground(c_a:getTheme("FrameBG"))
cbc:setForeground(c_a:getTheme("FrameText"))else aaa.width,aaa.height=bca.getSize()
cbc:setBackground(_aa.getTheme("BasaltBG"))
cbc:setForeground(_aa.getTheme("BasaltText"))end;_bb=true end end}
for cbc,dbc in pairs(_c)do _ba["add"..cbc]=function(_cc,acc)
return ddb(dbc(acc or _d(),_cc))end end;setmetatable(_ba,aaa)return _ba end
end; 
...
cd=true;return baa end,disable=function(baa)cd=false;return baa end,isEnabled=function(baa)return cd end,generateXMLEventFunction=function(baa,caa,daa)
...
baa.parent:drawBackgroundBox(caa-1,daa,1,aba,baa.bgColor)
baa.parent:drawForegroundBox(caa-1,daa,1,aba,ad["left"])end
if(ad["left"]~=false)and(ad["top"]~=false)then baa.parent:drawTextBox(
caa-1,daa-1,1,1,"\151")baa.parent:drawBackgroundBox(
caa-1,daa-1,1,1,baa.bgColor)baa.parent:drawForegroundBox(
caa-1,daa-1,1,1,ad["left"])end
if(ad["top"]~=false)then
baa.parent:drawTextBox(caa,daa-1,_ba,1,"\131")
baa.parent:drawBackgroundBox(caa,daa-1,_ba,1,baa.bgColor)
baa.parent:drawForegroundBox(caa,daa-1,_ba,1,ad["top"])end
if(ad["top"]~=false)and(ad["right"]~=false)then baa.parent:drawTextBox(
caa+_ba,daa-1,1,1,"\148")
baa.parent:drawForegroundBox(
caa+_ba,daa-1,1,1,baa.bgColor)
baa.parent:drawBackgroundBox(caa+_ba,daa-1,1,1,ad["right"])end
if(ad["right"]~=false)then
baa.parent:drawTextBox(caa+_ba,daa,1,aba,"\149")
baa.parent:drawForegroundBox(caa+_ba,daa,1,aba,baa.bgColor)
baa.parent:drawBackgroundBox(caa+_ba,daa,1,aba,ad["right"])end
if(ad["right"]~=false)and(ad["bottom"]~=false)then baa.parent:drawTextBox(
caa+_ba,daa+aba,1,1,"\133")
baa.parent:drawForegroundBox(
caa+_ba,daa+aba,1,1,baa.bgColor)
baa.parent:drawBackgroundBox(caa+_ba,daa+aba,1,1,ad["right"])end
if(ad["bottom"]~=false)then
baa.parent:drawTextBox(caa,daa+aba,_ba,1,"\143")
baa.parent:drawForegroundBox(caa,daa+aba,_ba,1,baa.bgColor)
baa.parent:drawBackgroundBox(caa,daa+aba,_ba,1,ad["bottom"])end
if(ad["bottom"]~=false)and(ad["left"]~=false)then baa.parent:drawTextBox(
caa-1,daa+aba,1,1,"\138")
baa.parent:drawForegroundBox(
caa-1,daa+aba,1,1,baa.bgColor)
baa.parent:drawBackgroundBox(caa-1,daa+aba,1,1,ad["left"])end end;d_a=false;return true end;return false end,getAbsolutePosition=function(baa,caa,daa)
if(
...
local daa=require("basaltEvent")()
local _ba=require("Frame")local aba=require("theme")local bba=require("utils")
local cba=require("basaltLogs")local dba=bba.uuid;local _ca=bba.createText;local aca=bba.tableCount
local bca=term.current()local cca="1.6.2"local dca=true
local _da=fs.getDir(table.pack(...)[2]or"")local ada,bda,cda,dda,__b,a_b={},{},{},{},{},{}local b_b,c_b,d_b,_ab;local aab={}if not term.isColor or not
term.isColor()then
error('Basalt requires an advanced (golden) computer to run.',0)end;local function bab()_ab=false
bca.clear()bca.setCursorPos(1,1)end;local cab=function(bcb,ccb)
__b[bcb]=ccb end
local dab=function(bcb)return __b[bcb]end;local _bb=function(bcb)aba=bcb end
local abb=function(bcb)return aba[bcb]end
local bbb={getMainFrame=function()return b_b end,setVariable=cab,getVariable=dab,getTheme=abb,setMainFrame=function(bcb)b_b=bcb end,getActiveFrame=function()return c_b end,setActiveFrame=function(bcb)c_b=bcb end,getFocusedObject=function()return
d_b end,setFocusedObject=function(bcb)d_b=bcb end,getMonitorFrame=function(bcb)
return cda[bcb]or dda[bcb][1]end,setMonitorFrame=function(bcb,ccb,dcb)if(b_b==ccb)then b_b=nil end;if(dcb)then
dda[bcb]={ccb,sides}else cda[bcb]=ccb end
if(ccb==nil)then dda[bcb]=nil end end,getBaseTerm=function()
return bca end,stop=bab,newFrame=_ba,getDirectory=function()return _da end}
local cbb=function(bcb)bca.clear()bca.setBackgroundColor(colors.black)
bca.setTextColor(colors.red)local ccb,dcb=bca.getSize()
if(aab.logging)then cba(bcb,"Error")end;local _db=_ca("Basalt error: "..bcb,ccb)local adb=1;for bdb,cdb in pairs(_db)do
bca.setCursorPos(1,adb)bca.write(cdb)adb=adb+1 end;bca.setCursorPos(1,
adb+1)_ab=false end
local function dbb(bcb,ccb,dcb,_db,adb)
if(#a_b>0)then local bdb={}
for n=1,#a_b do
if(a_b[n]~=nil)then
if
(coroutine.status(a_b[n])=="suspended")then
local cdb,ddb=coroutine.resume(a_b[n],bcb,ccb,dcb,_db,adb)if not(cdb)then cbb(ddb)end else table.insert(bdb,n)end end end
for n=1,#bdb do table.remove(a_b,bdb[n]- (n-1))end end end
local function _cb()if(_ab==false)then return end;if(b_b~=nil)then b_b:draw()
b_b:updateTerm()end
for bcb,ccb in pairs(cda)do ccb:draw()ccb:updateTerm()end
for bcb,ccb in pairs(dda)do ccb[1]:draw()ccb[1]:updateTerm()end end
local function acb(bcb,ccb,dcb,_db,adb)if
(daa:sendEvent("basaltEventCycle",bcb,ccb,dcb,_db,adb)==false)then return end
if(b_b~=nil)then
if(bcb=="mouse_click")then
b_b:mouseHandler(ccb,dcb,_db,false)c_b=b_b elseif(bcb=="mouse_drag")then b_b:dragHandler(ccb,dcb,_db,adb)
c_b=b_b elseif(bcb=="mouse_up")then b_b:mouseUpHandler(ccb,dcb,_db,adb)c_b=b_b elseif(bcb==
"mouse_scroll")then b_b:scrollHandler(ccb,dcb,_db,adb)c_b=b_b end end
if(bcb=="monitor_touch")then if(cda[ccb]~=nil)then
cda[ccb]:mouseHandler(1,dcb,_db,true)c_b=cda[ccb]end
if(aca(dda)>0)then for bdb,cdb in pairs(dda)do
cdb[1]:mouseHandler(1,dcb,_db,true,ccb)end end end
if(bcb=="char")then if(c_b~=nil)then c_b:charHandler(ccb)end end;if(bcb=="key_up")then if(c_b~=nil)then c_b:keyUpHandler(ccb)end
ada[ccb]=false end
if(bcb=="key")then if(c_b~=nil)then
c_b:keyHandler(ccb,dcb)end;ada[ccb]=true end
if(bcb=="terminate")then if(c_b~=nil)then c_b:eventHandler(bcb)
if(_ab==false)then return end end end
if



(bcb~="mouse_click")and(bcb~="mouse_up")and(bcb~="mouse_scroll")and(bcb~="mouse_drag")and(bcb~="key")and(bcb~="key_up")and(bcb~=
"char")and(bcb~="terminate")then
for bdb,cdb in pairs(bda)do cdb:eventHandler(bcb,ccb,dcb,_db,adb)end end;dbb(bcb,ccb,dcb,_db,adb)_cb()end
aab={logging=false,setTheme=_bb,getTheme=abb,drawFrames=_cb,getVersion=function()return cca end,setVariable=cab,getVariable=dab,setBaseTerm=function(bcb)bca=bcb end,log=function(...)cba(...)end,autoUpdate=function(bcb)
_ab=bcb;if(bcb==nil)then _ab=true end;local function ccb()_cb()while _ab do
acb(os.pullEventRaw())end end
local dcb,_db=xpcall(ccb,debug.traceback)if not(dcb)then cbb(_db)return end end,update=function(bcb,ccb,dcb,_db,adb)
if(
bcb~=nil)then
local bdb,cdb=xpcall(acb,debug.traceback,bcb,ccb,dcb,_db,adb)if not(bdb)then cbb(cdb)return end end end,stop=bab,stopUpdate=bab,isKeyDown=function(bcb)if(
ada[bcb]==nil)then return false end;return ada[bcb]end,getFrame=function(bcb)for ccb,dcb in
pairs(bda)do if(dcb.name==bcb)then return dcb end end end,getActiveFrame=function()return
c_b end,setActiveFrame=function(bcb)
if(bcb:getType()=="Frame")then c_b=bcb;return true end;return false end,onEvent=function(...)
for bcb,ccb in
pairs(table.pack(...))do if(type(ccb)=="function")then
daa:registerEvent("basaltEventCycle",ccb)end end end,schedule=function(bcb)
assert(bcb~=
"function","Schedule needs a function in order to work!")return
function(...)local ccb=coroutine.create(bcb)
local dcb,_db=coroutine.resume(ccb,...)if(dcb)then table.insert(a_b,ccb)else cbb(_db)end end end,createFrame=function(bcb)bcb=
bcb or dba()
for dcb,_db in pairs(bda)do if(_db.name==bcb)then return nil end end;local ccb=_ba(bcb,nil,nil,bbb)ccb:init()
table.insert(bda,ccb)if
(b_b==nil)and(ccb:getName()~="basaltDebuggingFrame")then ccb:show()end;return ccb end,removeFrame=function(bcb)bda[bcb]=
nil end,setProjectDir=function(bcb)_da=bcb end,debug=function(...)local bcb={...}if(b_b==nil)then
print(...)return end;if(b_b.name~="basaltDebuggingFrame")then
if
(b_b~=aab.debugFrame)then aab.debugLabel:setParent(b_b)end end;local ccb=""for dcb,_db in pairs(bcb)do
ccb=ccb..
tostring(_db).. (#bcb~=dcb and", "or"")end
aab.debugLabel:setText("[Debug] "..ccb)for dcb,_db in pairs(_ca(ccb,aab.debugList:getWidth()))do
aab.debugList:addItem(_db)end
if(
aab.debugList:getItemCount()>50)then aab.debugList:removeItem(1)end
aab.debugList:setValue(aab.debugList:getItem(aab.debugList:getItemCount()))if
(aab.debugList.getItemCount()>aab.debugList:getHeight())then
aab.debugList:setOffset(aab.debugList:getItemCount()-
aab.debugList:getHeight())end
aab.debugLabel:show()end}
aab.debugFrame=aab.createFrame("basaltDebuggingFrame"):showBar():setBackground(colors.lightGray):setBar("Debug",colors.black,colors.gray)
aab.debugFrame:addButton("back"):setAnchor("topRight"):setSize(1,1):setText("\22"):onClick(function()if(
aab.oldFrame~=nil)then aab.oldFrame:show()end end):setBackground(colors.red):show()
aab.debugList=aab.debugFrame:addList("debugList"):setSize("parent.w - 2","parent.h - 3"):setPosition(2,3):setScrollable(true):show()
aab.debugLabel=aab.debugFrame:addLabel("debugLabel"):onClick(function()
aab.oldFrame=b_b;aab.debugFrame:show()end):setBackground(colors.black):setForeground(colors.white):setAnchor("bottomLeft"):ignoreOffset():setZIndex(20):show()return aab